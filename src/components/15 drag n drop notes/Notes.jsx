...
const Notes = (props) => {
  ...
  useEffect(() => {
    ...
    const updatedNotes = notes.map((note) => {
      const savedNote = savedNotes.find((saveNote) => note.id === saveNote.id);
      if (savedNote) {
  ...
  const determinePosition = () => {
    const maxX = window.innerWidth - 17 * 16;
    const maxY = window.innerHeight - 17 * 16;
